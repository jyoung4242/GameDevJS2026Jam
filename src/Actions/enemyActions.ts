...
import { Enemy, FastEnemy, RangedEnemy } from "../Actors/enemies";
...
export class MoveCloserToSegment implements Action {
  id = nextActionId();
  private _tx: TransformComponent;
  private _motion: MotionComponent;
  public x!: number;
  public y!: number;
  private _owner: FastEnemy;
  private _start!: Vector;
  private _end: Vector | null = null;
  private _dir!: Vector;
  private _speed: number;
  ...
  private _started = false;
  private _stopped = false;
  constructor(owner: FastEnemy, speed: number) {
    this._tx = owner.get(TransformComponent);
    this._motion = owner.get(MotionComponent);
    // this._end = new Vector(destX, destY);
    this._speed = speed;
...
export class FindClosestSegment implements Action {
  id: number = nextActionId();
  ...
  isComplete(entity: any): boolean {
    let completeCheck = (this.owner as FastEnemy).targetSegment != null && this.owner.nodePath.length > 0;
    return completeCheck;
  }
  ...
  update(elapsed: number): void {
    if (!this.isStarted) {
      this.isStarted = true;
      this.isRunning = true;
      let segments: CableSegment[] | undefined = this.scene.entities.filter(e => e instanceof CableSegment);

      if (!segments) return;
      let closestSegment = null;
      let closestDistance = Infinity;
      for (const seg of segments) {
        let distance = seg.pos.distance(this.owner.pos);
        if (distance < closestDistance) {
          closestDistance = distance;
          closestSegment = seg;
        }
      ...
      (this.owner as FastEnemy).targetSegment = closestSegment;

      if (!(this.owner as FastEnemy).targetSegment) return;
      let targtNode = this.owner.getGraphNode((this.owner as FastEnemy).targetSegment!.pos);
      let currentNode = this.owner.getGraphNode(this.owner.pos);
      if (!currentNode || !targtNode) return;
      let astar = this.owner.navmap.aStar(currentNode, targtNode);
...
export class AttackSegment implements Action {
  id: number = nextActionId();
  isStarted: boolean = false;
  isRunning: boolean = false;
  owner: FastEnemy;
  segment: CableSegment | null = null;
  cooldownTimer: number = 0;
  flashColor: Color = Color.White;
  resetCooldown: number = 0;
  private _stopped: boolean = false;
  ...
  private _total: number = 0;
  private _currentDuration: number = 0;
  constructor(owner: FastEnemy, cooldown: number) {
    this.owner = owner;
    this.segment = owner.targetSegment;
    this.cooldownTimer = cooldown;
    this.resetCooldown = cooldown;
    this._graphics = owner.get(GraphicsComponent);
    this._duration = cooldown;
  ...
  update(elapsed: number): void {
    if (!this.isStarted) {
      ...
      this.owner.vel = Vector.Zero;
      // flash Player
      if (!this.owner.targetSegment) return;
